...
$("#loader").remove()}});$("#main #loader").fadeOut(400)}}function _t(a,c){value=i18n[a];if(null!=c)for(i=0;i<c.length;i++)value=value.replace("$"+(i+1),c[i]);return value}function toggleFolder(a,c){var d=$("ul",$(a).parent()),b=0;"none"==d.css("display")&&(b=1);d.slideToggle(200);$(a).html(b?"\u25bc":"\u25ba");!0==b?$(a).parent().addClass("isOpen"):$(a).parent().removeClass("isOpen")}
...
function readThis(a,c,d,b){"undefined"===typeof b&&(b=!1);var e=$("#pageTop").html(),f=$(a).parent().parent();$("#"+c).next();f.hasClass("eventRead")?"title"!=d&&$.ajax({url:"./action.php?action=unreadContent",data:{id:c},success:function(a){"noconnect"==a.status?alert(a.texte):(window.nbUnreadArticles++,$("#nbUnreadArticles").html(window.nbUnreadArticles),f.removeClass("eventRead"),""!=e&&"selectedFolder"!=e||$(window).data("nblus",$(window).data("nblus")-1),b&&b())}}):$.ajax({url:"./action.php?action=readContent",
data:{id:c},success:function(a){if("noconnect"==a.status)alert(a.texte);else switch(window.nbUnreadArticles--,$("#nbUnreadArticles").html(window.nbUnreadArticles),e){case "":f.addClass("eventRead");f.fadeOut(200,function(){b&&b();"display: none;"==$("article section:last").attr("style")&&$(window).scrollTop($(document).height())});$(window).data("nblus",$(window).data("nblus")+1);$("#nbarticle").html(parseInt($("#nbarticle").html())-1);break;case "selectedFolder":f.addClass("eventRead");$(window).data("nblus",
$(window).data("nblus")+1);b&&b();break;default:f.addClass("eventRead"),b&&b()}}})}
function toggleArticleDisplayMode(a,c){var d=$(".summary",$("#"+c).parent()),b=$(".content",$("#"+c).parent()),e=$(".articleDetails",$("#"+c).parent());0<d.length&&"display: none;"!=d.attr("style")?(action="content",0==$.trim(b.text()).length?$.ajax({url:"./action.php?action=articleDisplayMode&articleDisplayMode="+action+"&event_id="+c,success:function(c){"noconnect"==c.status?alert(c.texte):(console&&console.log&&""!=c&&console.log(c),d.hide(),b.html(c),b.show(200),a.innerHTML="-",a.title=_t("EVENT_DISPLAY_CONTENT"),
e.last().show(200))}}):(d.hide(),b.show(200),a.innerHTML="-",a.title=_t("EVENT_DISPLAY_CONTENT"),e.last().show(200))):0<b.length&&"display: none;"!=b.attr("style")?(b.hide(200),a.innerHTML="+",a.title=_t("EVENT_DISPLAY_TITLE"),1<e.length&&e.last().hide(200)):(action="summary",0==$.trim(d.text()).length?$.ajax({url:"./action.php?action=articleDisplayMode&articleDisplayMode="+action+"&event_id="+c,success:function(b){"noconnect"==b.status?alert(b.texte):(console&&console.log&&""!=b&&console.log(b),
d.html(b),d.show(200),a.innerHTML="++",a.title=_t("EVENT_DISPLAY_SUMMARY"))}}):(d.show(200),a.innerHTML="++",a.title=_t("EVENT_DISPLAY_SUMMARY")))}
function renameFeed(a,c){var d=$(a).parent().parent(),b=$("td.loopFoldersSettingsName",d),e=$("a",b).html(),d=$("td.loopFoldersSettingsLink",d),f=$("a",d).html(),g=b.attr("href");$(a).html(_t("SAVE"));$(a).attr("onclick","saveRenameFeed(this,"+c+',"'+g+'")');b.replaceWith('<td class="loopFoldersSettingsName"><input type="text" name="feedName" value="'+e+'"/></td>');d.replaceWith('<td class="loopFoldersSettingsLink shorten"><input type="text" name="feedUrl" value="'+f+'" /></td>')}
function saveRenameFeed(a,c,d){var b=$(a).parent().parent(),e=$('td.loopFoldersSettingsName input[name="feedName"]',b),f=e.val(),b=$('td.loopFoldersSettingsLink input[name="feedUrl"]',b),g=b.val();$(a).html(_t("RENAME"));$(a).attr("onclick","renameFeed(this,"+c+")");e.replaceWith('<a href="'+d+'">'+f+"</a>");b.replaceWith('<a href="'+g+'">'+g+"</a></span>");$.ajax({url:"./action.php?action=renameFeed",data:{id:c,name:f,url:g}})}
